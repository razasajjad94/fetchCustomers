...
/** Mongo customers collection — email fields checked in order (first non-empty wins) */
const MONGO_CUSTOMER_EMAIL_FIELDS = ["login_email", "customer_contact_email", "email"];
...
/**
 * Campaign.Name — prefers Mongo searchable_text (truncated 200 chars).
 * Assumption: legacy campaigns store a search blob, not a clean display name.
 */
function buildCampaignName(mongoDoc) {
  const text = (mongoDoc.searchable_text || "").toString().trim();
  if (text) {
    const short = text.slice(0, 200).replace(/\s+/g, " ");
    return short;
  }
  const id = normalizeObjectIdHex(mongoDoc._id);
  return `${DEFAULT_CAMPAIGN_NAME_PREFIX}-${id}`;
}
 ...
 */
function mapMongoCampaignToSqlV3Row(mongoDoc) {
  ...
  return {
    mongoCampaignId: normalizeObjectIdHex(mongoDoc._id),
    Name: buildCampaignName(mongoDoc),
...
export async function migrateOneCampaign(pool, mongoCampaignId, options = {}) {
  ...
  const sqlRow = mapMongoCampaignToSqlV3Row(mongoDoc);
